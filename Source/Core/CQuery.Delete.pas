...
type
  TCQueryDelete = class(TCQuerySection, ICQueryDelete)
  strict private
...
procedure TCQueryDelete.Clear;
begin
  FTableNames.Clear;
end;

constructor TCQueryDelete.Create;
begin
  inherited Create('Delete');
  FTableNames := TCQueryNames.Create;
end;

function TCQueryDelete.IsEmpty: Boolean;
begin
  Result := FTableNames.IsEmpty;
end;

function TCQueryDelete.Serialize: String;
begin
  if IsEmpty then
    Result := ''
  else
...
function TCQueryDelete.TableNames: ICQueryNames;
begin
  Result := FTableNames;
end;
