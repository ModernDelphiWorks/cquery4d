...
type
  ...
  private
    ...
    function GetPathCQLBrInc: TFileName;
  public
...
begin
  ...
  DirCQLBr := IncludeTrailingPathDelimiter(edtDirDestino.Text);
  ...
  PathOrigem  := DirCQLBr + 'DLLs\' + ANomeArquivo;
...
var
  ...
  PathFonte: String;
begin
  with FCQLBr.Installations[iVersion] do
  begin
    ...
    try
      ...
      // verificar se existe algo do CQLBr e remover do environment variable PATH do delphi
...
var
  ...
  const
    LISTA_PROIBIDOS: ARRAY[0..6] OF String = (
...
begin
  ...
  AddLibraryPathToDelphiPath(sDirLibrary, 'cqlbr');
end;
...
begin
  ...
  sVersao  := AnsiUpperCase(FCQLBr.Installations[iVersion].VersionNumberStr);
...
begin
  ...
  Sender.AddPathOption('U', FCQLBr.Installations[iVersion].LibFolderName[tPlatform]);
  Sender.AddPathOption('U', FCQLBr.Installations[iVersion].LibrarySearchPath[tPlatform]);
  Sender.AddPathOption('U', sDirLibrary);
  // -I<paths> = Include directories
  Sender.AddPathOption('I', FCQLBr.Installations[iVersion].LibrarySearchPath[tPlatform]);
  // -R<paths> = Resource directories
  Sender.AddPathOption('R', FCQLBr.Installations[iVersion].LibrarySearchPath[tPlatform]);
  ...
  begin
     ...
     if VersionNumberStr = 'd7' then
        Sender.AddPathOption('U', FCQLBr.Installations[iVersion].RootDir + '\Rave5\Lib');
...
begin
  ...
  FCQLBr := TJclBorRADToolInstallations.Create;

  // popular o combobox de versões do delphi instaladas na máquina
  for iFor := 0 to FCQLBr.Count - 1 do
  begin
    if      FCQLBr.Installations[iFor].VersionNumberStr = 'd3' then
      edtDelphiVersion.Items.Add('Delphi 3')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd4' then
      edtDelphiVersion.Items.Add('Delphi 4')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd5' then
      edtDelphiVersion.Items.Add('Delphi 5')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd6' then
      edtDelphiVersion.Items.Add('Delphi 6')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd7' then
      edtDelphiVersion.Items.Add('Delphi 7')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd9' then
      edtDelphiVersion.Items.Add('Delphi 2005')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd10' then
      edtDelphiVersion.Items.Add('Delphi 2006')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd11' then
      edtDelphiVersion.Items.Add('Delphi 2007')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd12' then
      edtDelphiVersion.Items.Add('Delphi 2009')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd14' then
      edtDelphiVersion.Items.Add('Delphi 2010')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd15' then
      edtDelphiVersion.Items.Add('Delphi XE')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd16' then
      edtDelphiVersion.Items.Add('Delphi XE2')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd17' then
      edtDelphiVersion.Items.Add('Delphi XE3')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd18' then
      edtDelphiVersion.Items.Add('Delphi XE4')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd19' then
      edtDelphiVersion.Items.Add('Delphi XE5')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd20' then
      edtDelphiVersion.Items.Add('Delphi XE6')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd21' then
      edtDelphiVersion.Items.Add('Delphi XE7')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd22' then
      edtDelphiVersion.Items.Add('Delphi XE8')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd23' then
      edtDelphiVersion.Items.Add('Delphi 10 Seattle')
    else if FCQLBr.Installations[iFor].VersionNumberStr = 'd24' then
    ...
    FCQLBr.Installations[iFor].OutputCallback := OutputCallLine;
  end;
  //
...
begin
  ...
  try
    ...
    begin
      ...
      for I := ListaPaths.Count - 1 downto 0 do
      begin
        if Pos('CQLBR', AnsiUpperCase(ListaPaths[I])) > 0 then
          ListaPaths.Delete(I);
      end;
      ...
      begin
        if Pos('CQLBR', AnsiUpperCase(IdePackages.PackageFileNames[I])) > 0 then
...
function TfrmPrincipal.GetPathCQLBrInc: TFileName;
begin
  Result := IncludeTrailingPathDelimiter(edtDirDestino.Text) + 'Source\cqlbr.inc';
end;
...
var
  ...
  procedure LigarDefineCQLBrInc(const ADefineName: String; const Aligar: Boolean);
  var
    F: TStringList;
    I: Integer;
  begin
    ...
    try
      F.LoadFromFile(GetPathCQLBrInc);
...
//  LigarDefineCQLBrInc('DRIVERRESTFUL', True);
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if FCQLBr.Installations[iVersion].CompilePackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if FCQLBr.Installations[iVersion].InstallPackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
                  ...
                  begin
                    ...
                    if FCQLBr.Installations[iVersion].UninstallPackage(sDirPackage + NomePacote, sDirLibrary, sDirLibrary) then
...
begin
  iVersion := edtDelphiVersion.ItemIndex;
  sPathBin := IncludeTrailingPathDelimiter(FCQLBr.Installations[iVersion].BinFolderName);
  // -- Plataforma só habilita para Delphi XE2
  // -- Desabilita para versão diferente de Delphi XE2
  //edtPlatform.Enabled := FCQLBr.Installations[iVersion].VersionNumber >= 9;
  //if FCQLBr.Installations[iVersion].VersionNumber < 9 then
  edtPlatform.ItemIndex := 0;
end;

// abrir o endereço do CQLBr quando clicar na propaganda
procedure TfrmPrincipal.imgPropaganda1Click(Sender: TObject);
begin
  // ir para o endereço do CQLBr
...
begin
  ...
  {$IFNDEF DEBUG}
  if FCQLBr.AnyInstanceRunning then
  begin
    Stop := True;
    Application.MessageBox(
...
begin
  ...
  // precisa ser no mesmo diretório que os fontes do CQLBr esteja.
  fDir := IncludeTrailingPathDelimiter(edtDirDestino.Text);
  fDir := fDir + 'Source\cqlbr.inc';
  if not FileExists(fDir) then
  begin
    Stop := True;
    edtDirDestino.SetFocus;
    Application.MessageBox(
      'Diretório de instalação selecionado, não contém os fontes do CQLBr.',
      'Erro.',
      MB_OK + MB_ICONERROR
    );
  end;
...
begin
  if MatchText(FCQLBr.Installations[clbDelphiVersion.ItemIndex].VersionNumberStr, ['d3','d4','d5','d6','d7','d9','d10','d11','d12','d13']) then
  begin
    Application.MessageBox(
      'Versão do delphi não suportada pelo CQLBr Components.',
      'Erro.',
      MB_OK + MB_ICONERROR
    );
  end;
